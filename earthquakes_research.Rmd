...
This allows us to test the hypothesis about the probabilities of the magnitude categories ("High," "Medium," "Low") across different continents. Specifically, we test:
$H_0: p_i = p_i(\theta)$ vs $H_1: p_i$ are unrestricted(no fixed relationship to θ).
The test statistic used is:
$t := \sum_i \frac{(o_i - e_i)^2}{e_i}$, where$o_i = n_i$ are the observed frequencies in each category and $e_i$ are the expected frequencies under $h_0$. Under $H_0$, the statistic T approximately follows a $\chi^2_r$ distribution with r degrees of freedom, which is determined by the data structure.
...
The decision tree classifier achieved an accuracy of 78.05% in predicting earthquake alert levels using features like magnitude, depth, significance (sig), tsunami occurrence, and maximum estimated instrumental intensity (mmi). While the model performs well, some misclassifications, particularly in "orange" and "yellow" levels, highlight areas for improvement. The tree structure shows how features like significance and magnitude influence alert level predictions.

## How Decision Tree Classifiers Work:

A decision tree classifier is an algorithm that predicts classes by splitting data into branches based on feature thresholds.\

1.  **Root Node**: The process begins with a root node representing the entire dataset.\
2.  **Splitting**: Features are selected to divide data into branches, minimizing classification error.\
3.  **Stopping Criteria**: Splitting stops when nodes are pure, or a maximum depth is reached.\
4.  **Prediction**: New data traverses the tree to a leaf node, determining the predicted class.

The model effectively captures relationships between earthquake features and alert levels. While promising, further refinement (e.g., pruning or ensemble methods) can improve performance.
